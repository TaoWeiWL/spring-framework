...
final class PostProcessorRegistrationDelegate {
	...
	public static void invokeBeanFactoryPostProcessors(
		...
		if (beanFactory instanceof BeanDefinitionRegistry) {
			...
			// 此处注意区分，两个接口是不同的，BeanDefinitionRegistryPostProcessor是BeanFactoryPostProcessor的子集
			...
			while (reiterate) {
				reiterate = false;
				// 找出所有实现BeanDefinitionRegistryPostProcessor接口的类，但是排除已经执行过的
				postProcessorNames = beanFactory.getBeanNamesForType(BeanDefinitionRegistryPostProcessor.class, true, false);
				// 遍历执行
				for (String ppName : postProcessorNames) {
		...
		// 到此为止，入参beanFactoryPostProcessors和容器中的所有BeanDefinitionRegistryPostProcessor已经全部处理完毕，
		// 下面处理容器中所有的BeanFactoryPostProcessor，可能包含一些实现类，只实现了BeanFactoryPostProcessor，
		// 并没有实现BeanDefinitionRegistryPostProcessor接口
		// Do not initialize FactoryBeans here: We need to leave all regular beans
		// uninitialized to let the bean factory post-processors apply to them!
	 ...
	 *
	 * @param postProcessors 需要排序的postProcessors
	...
	private static void sortPostProcessors(List<?> postProcessors, ConfigurableListableBeanFactory beanFactory) {
		...
		if (beanFactory instanceof DefaultListableBeanFactory) {
			// 获取beanFactory的DependencyComparator
