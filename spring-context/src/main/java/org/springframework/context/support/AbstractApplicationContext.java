...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	public void refresh() throws BeansException, IllegalStateException {
		synchronized (this.startupShutdownMonitor) {
			...
			// 容器刷新前的准备工作，即准备容器刷新的上下文
			prepareRefresh();

			// Tell the subclass to refresh the internal bean factory.
			// 创建容器对象：DefaultListableBeanFactory
			// 加载xml配置文件的属性值到当前工厂中，封装为BeanDefinition对象
			...
			try {
				...
				// 调用各种beanFactoryPostProcessor（后置处理器/增强器），对beanDefinition进行额外的后置处理
				// 注意区别BeanFactoryPostProcessor和BeanDefinitionRegistryPostProcessor
				invokeBeanFactoryPostProcessors(beanFactory);

				// Register bean processors that intercept bean creation.
				// 注册beanPostProcessor，此处仅是注册，调用处在getBean方法
				registerBeanPostProcessors(beanFactory);

				// Initialize message source for this context.
				// 为context初始化message源，实现国际化处理
				initMessageSource();
	...
	protected ConfigurableListableBeanFactory obtainFreshBeanFactory() {
		// 初始化BeanFactory，并进行XML文件读取，并将初始化的BeanFactory赋值给对应实体（即AbstractApplicationContext的子类）的属性
		refreshBeanFactory();
		// 返回对应实体的beanFactory属性
		return getBeanFactory();
	}
	...
	protected void invokeBeanFactoryPostProcessors(ConfigurableListableBeanFactory beanFactory) {
		// 获取到当前应用程序上下文的beanFactoryPostProcessors变量的值，实例化并调用执行所有已经注册的beanFactoryPostProcessor
		// 默认情况下，通过getBeanFactoryPostProcessors()来获取已经注册的beanFactoryPostProcessors，默认是空的，可以实现并进行扩展
	...
	protected void initMessageSource() {
		...
		if (beanFactory.containsLocalBean(MESSAGE_SOURCE_BEAN_NAME)) {
			...
			// 当父类beanFactory不为空，并且这个bean对象是HierarchicalMessageSource类型
	...
	protected void initApplicationEventMulticaster() {
		// 获取当前beanFactory,一般是DefaultListableBeanFactory
		ConfigurableListableBeanFactory beanFactory = getBeanFactory();
		// 判断容器中是否存在beanName为applicationEventMulticaster的bean，
	...
	/**
	 * 模板方法，没有添加任何实现，子类可以自行实现以添加特殊的容器刷新处理
